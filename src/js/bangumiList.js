...
export class BangumiList extends HTMLElement {
  traditionalized(cc) {
    const zhSimp =
    ...
    for (var i = 0; i < cc.length; i++) {
      if (zhSimp.indexOf(cc.charAt(i)) != -1)
        str += zhTrad.charAt(zhSimp.indexOf(cc.charAt(i)));
  ...
  constructor() {
    ...
    this._data.subscribe((val) => this.dataCallback(val));

    this._filteredData = new Observable([]);
    this._filteredData.subscribe((val) => this.dataCallback(val));
  ...
  pageCallback(val) {
    const len = this._filteredData.value.length;
    const start = (val - 1) * this._itemPerPage;
    let end = start + this._itemPerPage;
    if (end > len) {
      end = len;
    }
    this._currentPageData = this._filteredData.value.slice(start, end);
    this.build();
  }

  dataCallback(val) {
    this._filteredData.value = val;

    const len = this._filteredData.value.length;
    const start = (this._page.value - 1) * this._itemPerPage;
    let end = start + this._itemPerPage;
    if (end > len) {
      end = len;
    }
  ...
  searchCallback(searchValue) {
    const filterType = this.typeFilter.choice;

    this._filteredData.value = this._data.value.filter((e) => {
      const searchContent = this.traditionalized(
        searchValue.trim().toLowerCase()
      );
      const title = this.traditionalized(e.title.trim().toLowerCase());

      const season_id = e.season_id ? e.season_id.toString() : "";
      const media_id = e.media_id ? e.media_id.toString() : "";

      const acg_sn = e.acg_sn ? e.acg_sn.toString() : "";
      const anime_sn = e.anime_sn ? e.anime_sn.toString() : "";

      const splits = searchContent.split(" ");
        ...
        if (
          !(
            (title.includes(split) ||
              ...
              (e.is_exclusive && split.includes("獨家")) ||
              (e.is_vip && split.includes("會員")) ||
              (e.is_bilingual && split.includes("雙語"))) &&
            (filterType == 0 ||
              (filterType != 0 && e.type && filterType == e.type))
          )
        ) {
          found = false;
        }
    ...
    if (this._filteredData.value.length == 0) {
      this._page.value = 0;
    } else {
      this._page.value = 1;
    }
  ...
  buildPageGroup() {
    const pageGroup = Wrapper.generate("div");
    const prevBtn = Wrapper.generate("button", "prev");
    prevBtn.element.addEventListener("click", () => {
      this._page.value -= 1;
    });
    prevBtn.element.disabled = this._page.value <= 1;
    pageGroup.appendChild(prevBtn);
    const pageStatus1 = Wrapper.generate(
      "span",
      this._page.value +
        " / " +
  ...
  build() {
    const tableContainer = Wrapper.generate("div").addClass("table-container");

    const innerTable = Wrapper.generate("div").addClass("inner-table");

    // table container
    innerTable.appendChild(Wrapper.generate("h1", this._title));
    innerTable.appendChild(this.buildSearchGroup());
    innerTable.appendChild(this.buildPageGroup());
    innerTable.appendChild(this.buildTable());
    innerTable.appendChild(this.buildPageGroup());

    tableContainer.appendChild(innerTable);
    this.innerHTML = "";
    this.appendChild(tableContainer.element);

    return this;
  }
}

export const registerBangumiList = () =>
  customElements.define("bangumi-list", BangumiList);
